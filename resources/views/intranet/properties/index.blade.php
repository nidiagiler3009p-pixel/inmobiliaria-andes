...
<div class="w-full px-4 sm:px-6 lg:px-8 pt-1 pb-16 space-y-3 font-sans" 
    ...
    <div class="flex flex-wrap justify-between items-center bg-white/95 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-emerald-100 shadow-sm gap-2">
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('properties.create') }}" class="px-3.5 py-1.5 bg-[#2C4A3E] text-white text-[11px] font-extrabold rounded-xl shadow-sm hover:bg-emerald-800 transition flex items-center gap-1.5 shrink-0">
            ...
            <form method="GET" action="{{ route('properties.index') }}" class="flex items-center shrink-0">
                ...
                <select name="agent_id" onchange="this.form.submit()" class="bg-[#FDFBF7] border border-emerald-200 rounded-xl px-3 py-1.5 text-[11px] font-bold text-[#2C4A3E] focus:outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer shadow-sm">
                    <option value="">Tus Propiedades ▾</option>
                    <option value="all" {{ request('agent_id') == 'all' ? 'selected' : '' }}>Catálogo Global Empresa</option>
                    @isset($agents)
        ...
        <div x-show="sidebarOpen" 
            ...
            <div class="flex justify-between items-center border-b border-emerald-100 pb-3">
                ...
                <div class="flex items-center gap-2">
                    <a href="{{ route('properties.index') }}" class="text-[11px] font-bold text-gray-400 hover:text-emerald-700 transition">Limpiar ⟲</a>
            ...
            <form method="GET" action="{{ route('properties.index') }}" class="space-y-4 text-xs font-bold">
                ...
                <div class="pt-2 border-t border-emerald-100">
                    ...
                    <div class="space-y-1">
                        ...
                        @foreach($propertyTypes as $type)
                            <a href="{{ route('properties.index', array_merge(request()->all(), ['property_type' => $type['name']])) }}" class="flex justify-between items-center px-3 py-2 rounded-xl hover:bg-emerald-50 transition {{ request('property_type') == $type['name'] ? 'bg-emerald-100/70 text-emerald-900 font-extrabold shadow-2xs' : 'text-gray-600' }}">
                ...
                <div class="space-y-2 pt-2 border-t border-emerald-100">
                    ...
                    <div class="space-y-1.5">
                        @php
                            $featuresList = [
                                ['id' => 'parqueadero', 'label' => 'Parqueadero', 'count' => $countParq ?? 0],
                        ...
                        @foreach($featuresList as $feat)
                            <label class="flex justify-between items-center px-2 py-1 rounded-lg hover:bg-emerald-50 cursor-pointer transition">
                                <span class="flex items-center gap-2 font-normal text-gray-700">
                                    <input type="checkbox" name="features[]" value="{{ $feat['id'] }}" 
                                        {{ is_array(request('features')) && in_array($feat['id'], request('features')) ? 'checked' : '' }}
                                        class="rounded border-emerald-300 text-[#2C4A3E] focus:ring-emerald-500 w-4 h-4">
                                    {{ $feat['label'] }}
                                </span>
                                <span class="bg-[#2C4A3E] text-white text-[9px] px-2 py-0.5 rounded-full font-bold">{{ $feat['count'] }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
